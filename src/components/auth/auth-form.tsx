...
export function AuthForm({ mode, action, googleError }: AuthFormProps) {
    ...
    <Card className="glass-panel w-full max-w-md border-0">
      <CardHeader className="text-center">
        <CardTitle className="text-3xl">
          {isLogin ? ro.auth.welcomeBack : ro.auth.createAccount}
        </CardTitle>
        <CardDescription>
          {isLogin ? ro.auth.loginDesc : ro.auth.signupDesc}
        </CardDescription>
      </CardHeader>
        ...
        <form action={formAction} className="space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={pending}>
            {pending
              ...
              : isLogin
                ? ro.auth.signIn
                : ro.auth.createAccountBtn}
          </Button>
        </form>

        <p className="text-center text-sm text-muted-foreground">
          {isLogin ? ro.auth.newToEvento : ro.auth.hasAccount}{" "}
          <Link
            href={isLogin ? "/signup" : "/login"}
            className="font-medium text-primary hover:underline"
          >
            {isLogin ? ro.auth.createAccountLink : ro.auth.signInLink}
          </Link>
        </p>
      </CardContent>
    </Card>
