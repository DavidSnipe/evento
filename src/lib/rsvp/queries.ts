...
export async function getHouseholdBundle(
  ...
  const members: InvitationMemberWithResponse[] = mapMemberRows(membersRes.data ?? []);
...
/** Public RSVP — no join to guests (avoids exposing contact/table data to anon). */
export async function getPublicHouseholdBundle(
  ...
  if (householdError || !household) {
    if (householdError) console.error("getPublicHouseholdBundle household:", householdError);
    return null;
  }
  ...
  if (membersRes.error) {
    console.error("getPublicHouseholdBundle members:", membersRes.error);
    return null;
  }

  const members: InvitationMemberWithResponse[] = mapMemberRows(membersRes.data ?? []).map(
    (member) => ({ ...member, guest: null })
  );
...
export async function getPublicHouseholdBundlesByEvent(
  eventId: string
): Promise<InvitationHouseholdBundle[]> {
  const households = await getHouseholdsByEvent(eventId);
  const bundles = await Promise.all(
    households.map((h) => getPublicHouseholdBundle(h.id))
  );
